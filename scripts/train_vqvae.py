...
def train_step(encoder, decoder, codebook, optimizer, restarter, r, loss_fn):
    ...
    with tf.GradientTape() as tape:
        ...
        # Compute audio loss using selected loss function
...
def main():
    ...
    while True:
        ...
        for step in range(args.epoch_steps):
            ...
            result = train_step(encoder, decoder, codebook, opt, restarter, batch, loss_fn)
        ...
        current_audio_loss = audio_loss_acc.get()
        
        # Save weights only if audio loss improved
        if current_audio_loss < best_audio_loss:
            prev_best = best_audio_loss
            best_audio_loss = current_audio_loss
            model_prefix = args.model
            if prev_best == float('inf'):
                print(f"Saving weights (initial save, {loss_name} loss: {current_audio_loss:.4e})")
            else:
                print(f"Saving weights ({loss_name} loss improved: {current_audio_loss:.4e} < {prev_best:.4e})")
            ...
            print(f"Skipping save ({loss_name} loss did not improve: {current_audio_loss:.4e} >= {best_audio_loss:.4e})")
