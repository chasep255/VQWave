...
class PositionalEmbedding(layers.Layer):
    """Learned positional embeddings for sequences up to max_seq_len."""
    
    def __init__(self, max_seq_len, width, **kwargs):
    ...
    def build(self, input_shape=None):
        self.embedding = self.add_weight(
            name='embedding', 
            shape=(self.max_seq_len, self.width),
            trainable=True
        )
        
    def call(self, x):
        # x: [batch, seq_len, ...] - we just need seq_len
        seq_len = tf.shape(x)[1]
        # Slice embeddings to match input sequence length
        pos_emb = self.embedding[:seq_len]
        # Broadcast to batch dimension
        return tf.broadcast_to(pos_emb, [tf.shape(x)[0], seq_len, self.width])
